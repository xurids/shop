...
function sum(){
	if ($("ul input[checked='checked']").length==$("li").length) {
		...
		$(".bottom input[type=checkbox]").next("img").attr("src","images/c_checkbox_on.png");
	}else{
		$(".bottom input[type=checkbox]").removeAttr("checked");
		$(".bottom input[type=checkbox]").next("img").attr("src","images/c_checkbox_off.png");
	}
}
/*判断是否全选*/
/*给单选框或复选框添加样式*/
...
$('.bottom-label input').change(function(){
		if ($(this).attr("checked")=="checked") {
			...
			$(".con input[type='checkbox']").next('img').attr("src","images/c_checkbox_off.png");
		}else{
			$(".con input[type='checkbox']").attr("checked","checked");
			$(".con input[type='checkbox']").next('img').attr("src","images/c_checkbox_on.png");
		}
		checkbox($(this));
})
/*底部全选*/
...
$("ul input[type='checkbox']").change(function(){
	...
	if($(this).parents('ul').find("input[checked='checked']").length==$(this).parents("ul").children('li').length){	
		...
		$ul_input.next('img').attr("src","images/c_checkbox_on.png");
	}else{
		$ul_input.removeAttr("checked");
		$ul_input.next('img').attr("src","images/c_checkbox_off.png");
	} 
	sum();
})
/*点击加一*/
...
$('.delete').click(function(){
	...
	$('input[type=checkbox]').next("img").attr("src","images/c_checkbox_on.png");
